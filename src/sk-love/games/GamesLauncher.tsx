...
const DEFAULT_CONFIG: GameConfig = {
  casino: { chips: [500, 1000, 5000, 10000, 50000, 1000000], timer_seconds: 10 },
  ferry: {
    chips: [500, 1000, 5000, 10000, 50000, 1000000],
    timer_seconds: 20,
    slots: [
  ...
  teenpatti: { chips: [500, 1000, 5000, 10000, 50000, 1000000], timer_seconds: 20, hands: ["A", "B", "C"] },
};
...
const toNumberArray = (value: unknown, fallback: number[]) => {
  ...
  if (numbers.length && numbers.some((n) => n >= 500)) {
    return numbers;
  }
  return fallback;
};
...
type Props = {
  ...
  compact?: boolean;
  /** Callback when user coin balance changes during gameplay */
...
export function GamesLauncher({ open, onClose, initialGame = null, compact = false, onBalance, onBalanceChange }: Props) {
  ...
  const handleBalanceUpdate = useCallback((bal: number) => {
    setBalance(bal);
    onBalance?.(bal);
    onBalanceChange?.(bal);
  ...
  // Compact = opened from Party Room / Video Room → 90vh full comfortable bottom sheet (shows everything with zero scroll)
  const shellSize = compact
    ? "w-full max-w-lg h-[min(90dvh,880px)] max-h-[calc(100dvh-env(safe-area-inset-top)-env(safe-area-inset-bottom))] rounded-t-3xl rounded-b-none border border-white/15"
    : "w-full h-full max-w-full h-[100dvh] max-h-[100dvh] rounded-none border-none";

  return (
    <div className={
      compact
        ? "fixed inset-0 z-[65000] flex items-end justify-center bg-black/40 backdrop-blur-[2px] p-0 sk-preserve-dark"
        : "fixed inset-0 z-[65000] flex items-center justify-center bg-black/95 backdrop-blur-md p-0 sk-preserve-dark"
    } onClick={onClose}>
      <div className={`relative w-full min-h-0 ${shellSize} overflow-hidden bg-neutral-950 shadow-2xl flex flex-col`} onClick={(e) => e.stopPropagation()}>
        ...
        {/* Header (Only shown when selecting games from menu) */}
        {!activeGame && (
          <div className="flex items-center justify-between px-3 py-2 bg-gradient-to-r from-fuchsia-600 via-pink-600 to-rose-600 shrink-0">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                onClick={onClose}
                className="w-7 h-7 rounded-full bg-black/30 hover:bg-black/50 text-white text-base flex items-center justify-center cursor-pointer"
                aria-label="Close"
              >
                ×
              </button>
        ...
        {/* Body */}
        <div className="min-h-0 flex-1 overflow-y-auto relative overscroll-contain">
          {error && (
