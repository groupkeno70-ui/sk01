...
export default function AdminPanel({
  ...
  const readGiftImageFile = (file: File | null, onLoaded: (dataUrl: string) => void) => {
    if (!file) return;
    const isImage = file.type.startsWith("image/");
    const isVideo = file.type.startsWith("video/");
    if (!isImage && !isVideo) {
      setGiftFormMsg("❌ Please select an image or video/animation file (PNG, JPG, GIF, WebP, MP4, WebM).");
      return;
    }
    if (file.size > 25 * 1024 * 1024) {
      setGiftFormMsg("❌ Gift media must be 25 MB or smaller.");
      return;
    }
    ...
    reader.onerror = () => setGiftFormMsg("❌ Gift media could not be read.");
        ...
        <div className="bg-slate-950 border border-slate-850 rounded-xl p-4 mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-3">
            ...
            <div className="md:col-span-2">
              ...
              />
              <label className="mt-1.5 flex cursor-pointer items-center justify-center gap-1 rounded border border-dashed border-slate-700 bg-slate-900 px-2 py-1.5 text-[9px] font-bold text-slate-300 hover:border-pink-500 hover:text-pink-300">
                ...
                <input
                  ...
                  className="hidden"
                  onChange={(e) => {
                    readGiftImageFile(e.target.files?.[0] || null, (dataUrl) =>
                      setNewGift((p) => ({ ...p, image: dataUrl })),
                    );
              ...
              {newGift.image && (
                <img src={newGift.image} alt="New gift preview" className="mt-1.5 h-10 w-10 rounded-lg object-cover" />
              )}
            </div>
            <div>
